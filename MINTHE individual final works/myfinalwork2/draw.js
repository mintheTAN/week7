...
function changeRotationSpeed() {
    if (frameCount % 50 === 0 && musicAndAnimationActive) {
        ...
        //the amplitude level is mapped from the range [0, 1] to a new range of [0.01, 50]. 
        rotationSpeed = lerp(rotationSpeed*5, targetSpeed*5, 5);//the rotation speed gradually changes in response to changes in music loudness.
        for (let i = 0; i < rows; i++) {
            for (let j = 0; j < cols; j++) {
                for (let k = 0; k < concentricCircles; k++) {
