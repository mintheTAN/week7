...
function preload() {
    song = loadSound("assets/circle of life.mp3"); //Loads the audio file "circle of life.mp3" into the song variable.
    //"Circle of Life" is the iconic song featured in Disney's animated masterpiece, "The Lion King." 
    //The song is famous  for its memorable opening lines, which were composed with the influence of South African music. 
    //While "Circle of Life" may evoke a distinct African ethnic flavor similar to the inspiration behind Pacita's artwork,
    // it also shares a common theme of reflection on the cyclical nature of destiny. 
}
...
function changeRotationSpeed() {
    if (frameCount % 50 === 0 && musicAndAnimationActive) {
        ...
        let targetSpeed = map(level, 0, 1, 0, 50);
        //the amplitude level is mapped from the range [0, 1] to a new range of [0.01, 20]. 
        rotationSpeed = lerp(rotationSpeed*5, targetSpeed*2, 0.1);//the rotation speed gradually changes in response to changes in music loudness.
        for (let i = 0; i < rows; i++) {
            for (let j = 0; j < cols; j++) {
                for (let k = 0; k < concentricCircles; k++) {
